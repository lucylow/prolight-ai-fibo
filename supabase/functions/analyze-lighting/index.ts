...
serve(async (req) => {
  ...
  try {
    const lightingSetup: Record<string, LightSettings> = await req.json();
    console.log("Analyzing lighting setup:", JSON.stringify(lightingSetup));

    const analysis = analyzeLighting(lightingSetup);
...
function analyzeLighting(lightingSetup: Record<string, LightSettings>) {
  ...
  const recommendations = generateRecommendations(keyFillRatio, key, fill, rim);

  return {
    keyFillRatio: Math.round(keyFillRatio * 100) / 100,
    lightingStyle,
    contrastScore: Math.round(contrastScore * 100) / 100,
    totalExposure: Math.min(Math.round(totalExposure * 100) / 100, 1.0),
    colorAnalysis: {
      averageTemperature: Math.round(avgTemp),
      harmonyScore: Math.round(harmonyScore * 100) / 100,
      assessment: harmonyScore > 0.7 ? "harmonious" : "contrasting"
    },
    professionalRating,
    recommendations
  };
...
function generateRecommendations(
  ...
  if (key && fill && Math.abs(key.colorTemperature - fill.colorTemperature) > 1000) {
    recommendations.push("Consider matching color temperatures for a more cohesive look");
  }
